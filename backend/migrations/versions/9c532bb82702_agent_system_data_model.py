...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            sa.Enum("main", "sourcing", "outreach", name="agentrole", native_enum=False, length=32),
            nullable=False,
        ),
    ...
    op.add_column(
        "campaign", sa.Column("next_source_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "campaign",
        sa.Column(
    ...
    op.add_column(
        "workspace",
        sa.Column("vertical", sa.String(length=50), server_default="recruiting", nullable=False),
    )
